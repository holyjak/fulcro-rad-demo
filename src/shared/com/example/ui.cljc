...
(defsc JhOrgDashboard [this {:fake/keys [details-router org-nr]}]
  {:query         [:fake/org-nr {:fake/details-router (comp/get-query JhOrgDetailsRouter)}]
   ...
   :pre-merge (fn [{:keys [data-tree current-normalized state-map]}]
                ;; Add the required link to the router into the component's data
                (merge (comp/get-initial-state JhOrgDashboard)
                       data-tree)
                #_(assoc data-tree
                    :fake/details-router (comp/get-initial-state JhOrgDetailsRouter {})))
